...
class FarmMoleBurrowWorldMockBukkitTest : FunSpec({
    ...
    beforeEach {
        ...
        for (x in -16..16) for (z in -16..16) for (y in 45..58) world.getBlockAt(x, y, z).type = Material.STONE
    }
    ...
    test("preview finds a bounded underground maze without mutating the world") {
        ...
        scene?.records?.all { it.originalData == "minecraft:stone" } shouldBe true
        scene?.records?.all { world.getBlockAt(it.x, it.y, it.z).type == Material.STONE } shouldBe true
    }
