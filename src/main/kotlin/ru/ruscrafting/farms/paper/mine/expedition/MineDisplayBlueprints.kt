    ...
    fun model(kind: String): List<Part> = buildList {
        ...
        fun wheel(cx:Float,cy:Float,cz:Float,r:Float, moving:Boolean=true) {
            ...
            repeat(16) { i ->
                ...
                box(Material.EXPOSED_CUT_COPPER,cx+cos(a)*r,cy+sin(a)*r,cz,.18f,r*.44f,.24f,a,moving,pivot)
                if(i%2==0) box(Material.IRON_BLOCK,cx+cos(a)*(r+.12f),cy+sin(a)*(r+.12f),cz,.25f,.22f,.32f,a,moving,pivot)
            }
            ...
            repeat(spokes) { i -> val a=i*PI.toFloat()*2/spokes
                box(if(r>=3) Material.STRIPPED_DARK_OAK_LOG else Material.POLISHED_ANDESITE,
                    cx+cos(a)*r/2,cy+sin(a)*r/2,cz,r,if(r>=3) .32f else .12f,if(r>=3) .32f else .16f,a,moving,pivot) }
            box(Material.IRON_BLOCK,cx,cy,cz,.34f,.34f,.55f)
        }
        fun frame(width:Float,depth:Float,height:Float) {
            ...
            for(x in listOf(-width/2,width/2)) for(z in listOf(-depth/2,depth/2)) {
                box(Material.POLISHED_ANDESITE,x,height/2,z,.22f,height,.22f)
        ...
        when(kind) {
            ...
            "sluice" -> {
                ...
                for(y in listOf(1f,2f,3f,4f)) box(Material.IRON_BLOCK,0f,y,-.34f,3.35f,.16f,.18f)
                box(Material.CUT_COPPER,0f,5.3f,-.6f,4.4f,.45f,1f)
                box(Material.IRON_BLOCK,0f,4.1f,.15f,.24f,2.8f,.24f)
                wheel(0f,2.5f,.75f,1.1f)
                for(x in listOf(-1.6f,1.6f)) box(Material.PRISMARINE_BRICKS,x,.35f,-2.1f,.6f,.7f,3.2f)
            ...
            "coal_bunker", "feed_hopper" -> {
                ...
                for(z in listOf(-1.5f,1.5f)) box(Material.WEATHERED_CUT_COPPER,0f,2.6f,z,3.6f,2.4f,.3f)
                box(Material.POLISHED_ANDESITE,0f,1.5f,0f,3.6f,.35f,3f)
                if(kind=="coal_bunker") {
            ...
            "casting_bed" -> {
                frame(5f,3.4f,1.5f)
                box(Material.POLISHED_BLACKSTONE,0f,1.45f,0f,4.5f,.4f,3.1f)
                for(x in listOf(-2f,2f)) box(Material.IRON_BLOCK,x,1.85f,0f,.3f,.8f,3.2f)
                for(z in listOf(-1.4f,1.4f)) box(Material.IRON_BLOCK,0f,1.85f,z,4f,.8f,.3f)
            ...
            "casting_rack" -> {
                ...
                for (x in listOf(-1.45f, 1.45f))
                    box(Material.YELLOW_TERRACOTTA, x, .8f, 1.2f, .3f, 1.6f, .3f)
            }
            "assembly_bench" -> {
                ...
                box(Material.POLISHED_BLACKSTONE,0f,1.7f,0f,1.8f,.4f,1.8f)
            ...
            "crane_console", "furnace_console" -> {
                frame(3.8f,2.2f,1.4f)
                box(Material.WEATHERED_CUT_COPPER,0f,1.65f,0f,3.8f,.8f,2.2f)
                box(Material.POLISHED_BLACKSTONE,0f,2.05f,-.65f,3.5f,1.1f,.25f)
                for(x in listOf(-1.2f,0f,1.2f)) {
                    box(Material.CYAN_STAINED_GLASS,x,2.22f,-.48f,.78f,.55f,.09f)
                    box(Material.SEA_LANTERN,x,2.22f,-.55f,.65f,.4f,.05f)
                }
                for(x in listOf(-1.15f,1.15f)) {
                    box(Material.POLISHED_BLACKSTONE,x,2.08f,.5f,.65f,.16f,.65f)
                    box(Material.IRON_BLOCK,x,2.42f,.5f,.13f,.7f,.13f,x*.2f,moving=true,pivot=Vector3f(x,2.08f,.5f),motion="lever")
                    box(if(kind=="crane_console") Material.RED_CONCRETE else Material.ORANGE_CONCRETE,
                        x,2.77f,.5f,.42f,.18f,.3f,moving=true,pivot=Vector3f(x,2.08f,.5f),motion="lever")
                }
            ...
            "furnace" -> {
                ...
                box(Material.POLISHED_BLACKSTONE,0f,.45f,0f,7f,.9f,6.8f)
            ...
            "waterwheel" -> {
                ...
                for(z in listOf(-2f,2f)) {
                    box(Material.STONE_BRICKS,0f,3f,z,1.4f,6f,1.4f)
            ...
            "tank" -> {
                ...
                box(Material.CYAN_STAINED_GLASS,0f,2.4f,.87f,.35f,2f,.08f)
                box(Material.SEA_LANTERN,0f,2.4f,.84f,.2f,1.8f,.05f)
            ...
            "winch" -> {
                ...
                box(Material.IRON_BLOCK,0f,2.6f,-.5f,.1f,2.5f,.1f)
            }
            "rack" -> {
            ...
            "console" -> {
                ...
                for(x in listOf(-.55f,0f,.55f)) {
                    box(Material.POLISHED_BLACKSTONE,x,1.4f,.6f,.4f,.35f,.08f)
