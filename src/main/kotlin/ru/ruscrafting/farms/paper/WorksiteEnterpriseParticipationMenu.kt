...
internal class WorksiteEnterpriseParticipationMenu(
    ...
    fun open(player: Player): FarmMenuSession? {
        val view = service.enterpriseParticipation(player.uniqueId) ?: run { back(player); return null }
        val personal = service.enterprisePlayerView(player.uniqueId) ?: run { back(player); return null }
        val week = personal.currentWeekStartEpochDay + 7
        return menus.open(player, MENU, { open(player) }) {
    ...
    private fun confirm(player: Player, plan: WorksiteEnterprisePlan, week: Long) {
        val personal = service.enterprisePlayerView(player.uniqueId) ?: run { open(player); return }
        if (week != personal.currentWeekStartEpochDay + 7 || !(personal.canVote || personal.canAdvise)) { open(player); return }
        val menu = ArcFarmsMenuPlatform.ENTERPRISE_CONFIRM
        menus.open(player, menu, { confirm(player, plan, week) }) {
            FarmMenuContent(title = text(player, "confirm-title"), background = menus.background(menu), elements = mapOf(
                id("confirm") to entry(menus.item(menu, id("confirm"), text(player, "confirm-name", mapOf("plan" to text(player, "${plan.name.lowercase()}.name"))), listOf(
                    ...
                    menus.transition(player, context.session) {
                        ...
                        service.voteEnterprise(player.uniqueId, plan, week) { saved ->
                            if (player.isOnline && expectedSession != null && menus.session(player) === expectedSession) {
                                player.sendMessage(text(player, if (saved) "saved" else "failed"))
                                open(player)
                            }
                        }
                        expectedSession = open(player)
                    }
                },
    ...
    ) = FarmMenuEntry(item, enabled, acceptedClicks, FarmMenuClickHandler(click))
