...
class ArcFarmsService(
    ...
    fun enterprisePlayerView(playerId: UUID) = enterprise.playerView(playerId)
    fun enterpriseParticipation(playerId: UUID) = enterprise.participationView(playerId)
    internal fun voteEnterprise(playerId: UUID, plan: ru.ruscrafting.farms.domain.enterprise.WorksiteEnterprisePlan, week: Long, complete: (Boolean) -> Unit) = enterprise.vote(playerId, plan, week, complete)
    ...
    fun onChunkUnload(chunk: org.bukkit.Chunk) = worksites.beforeChunkUnload(chunk)
