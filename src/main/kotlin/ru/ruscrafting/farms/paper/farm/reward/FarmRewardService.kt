...
internal class FarmRewardService(
    ...
    fun replace(pending: Collection<PendingFarmReward>, claimed: Map<String, Long>) = ledger.replace(pending, claimed)
    ...
    fun queueCompletion(runtime: FarmRuntime, contributors: Map<UUID, Int>) {
        ...
        val recipients = {
            accepted.mapNotNull { Bukkit.getPlayer(it.playerId) }
                .filter(Player::isOnline)
                .distinctBy(Player::getUniqueId)
        }
        if (saved) {
            deliverPending(recipients())
        } else {
            supervisor.runLater(20L) {
                if (operational()) deliverPending(recipients())
            }
        }
    }
    ...
    fun deliverPending(players: Collection<Player>) {
        ...
        val claim = ledger.claim(online.keys, persistBlocking)
        ...
        claim.rewards.forEach { reward -> online[reward.playerId]?.let { player -> deliverClaimed(player, reward) } }
    }
