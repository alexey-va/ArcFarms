...
этапа B. `SHADOW` по-прежнему калибрует выручку, а отложенная `LIVE`-ветка уже
...
Эндгейм-игроки финансируют компанию, владеют её акциями и получают дивиденды.
Обычные игроки выполняют тяжёлые заказы ArcFarms; владельцы могут давать
консультативные предложения по недельному плану.
...
Работник заранее видит обычную награду ArcFarms и расчётный worker bonus текущей
компании. Bonus durable учитывается в недельном распределении; интерфейс не
обещает немедленную денежную выплату.
...
Одна лицензия действует 12 бизнес-недель. Пока лицензия активна, cumulative
gross envelope открывается равными долями: в первой активной неделе доступно
`floor(M / 12)`, неиспользованный лимит переносится, а в последней активной
неделе доступен весь оставшийся envelope. Уже принятые reservations сохраняются
при смене pacing или reload.

Для каждой лицензии заранее фиксируется общий server-funded revenue envelope
`M`, причём:
...
Продление пока не реализуется и не обещается. После закрытия последней недели и
отсутствия reservations лицензия переходит в `EXPIRED`: остаток казны один раз
распределяется по долям, включая офлайн-владельцев, durable-кредитуется на их
инвестиционные счета и обнуляется. Автоматической конфискации, принудительной
продажи и дисконтированного buyback нет.
...
- после исчерпания коммерческой квоты реальные обычные completions всё равно
  учитываются для project milestones `10/25/50`; admin, idle и replay не
  учитываются.
...
вкладу и отражается в personal history. Он идёт поверх неизменённой обычной
награды ArcFarms. Повышение bonus
...
4. **План заказов:** один из трёх заранее собранных наборов: `STEADY` с короткими
   обычными заказами, `TEAM` со смешанным пулом (default) и `CHALLENGE` с
   редкими/сложными заказами и fallback к исходному пулу.
...
- владелец не менее 5% акций может подать один advisory proposal на следующую
  неделю;
...
- один подтверждённый vote на UUID действует для следующей недели; доли дают
  вес, quorum — 20% turnout;
- tie сохраняет текущую политику; работники и non-owners дают advisory feedback;
- если ballot не создан или quorum не достигнут, голосов нет и текущая политика
  сохраняется;
...
Детальные dividend instructions остаются planned feature; текущий MVP durable
кредитует claims на инвестиционный счёт без дополнительных автоматических
распоряжений. В будущем игрок сможет выбрать:
...
### Dormancy и выход владельца

Автоматической dormancy-продажи, принудительного offline exit и
дисконтированного buyback в текущем MVP нет. Акции и начисленные claims
сохраняются durable независимо от онлайна. Политика dormancy, вторичный auction
и renewal остаются отдельными будущими решениями.
...
Вторичный auction, dormancy и renewal остаются planned work. Их можно включать
только после отдельного решения по экономике, хранению и operator review.
...
- cumulative pacing первой активной недели открывает `1/12` envelope,
  неиспользованное переносится, а последняя активная неделя открывает остаток;
- accepted reservations сохраняются при смене pacing и reload;
- expiry после всех закрытых недель durable-liquidates treasury ровно один раз,
  включая offline owners;
- personal history показывает contributions, pending worker bonus,
  projected dividend и next close без обещания доходности;
